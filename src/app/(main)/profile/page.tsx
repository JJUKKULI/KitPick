      ...
      <div className="bg-surface border border-surface-border rounded-2xl p-6 mb-6">
        <div className="flex items-center gap-4">
          ...
          {profileLoading ? (
            ...
            <AvatarSection
              ...
              onAvatarChange={(url) => { setProfile((p) => p ? { ...p, avatar_url: url } : p); updateAvatarUrl(url); }}
            />
          )}
            ...
            {editingName ? (
              <div className="flex items-center gap-2 mb-1">
                <input
                  ...
                  onKeyDown={(e) => { if (e.key === 'Enter') handleNameSave(); if (e.key === 'Escape') { setEditingName(false); setNameError(''); setNewName(profile?.username ?? ''); } }}
        ...
        <div className="flex gap-6 mt-5 pt-5 border-t border-surface-border">
          <div className="text-center">
            <div className="text-xl font-bold text-white">{wishedProducts.length}</div>
            <div className="text-xs text-zinc-500 mt-0.5">찜한 키트</div>
          </div>
          <div className="w-px bg-surface-border" />
          <div className="text-center">
            <div className="text-xl font-bold text-white">0</div>
            <div className="text-xs text-zinc-500 mt-0.5">저장된 저널</div>
          </div>
          <div className="w-px bg-surface-border" />
          <div className="text-center">
            <div className="text-xl font-bold text-brand-400">$0</div>
            <div className="text-xs text-zinc-500 mt-0.5">예상 절약</div>
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-1 bg-surface border border-surface-border rounded-xl p-1 mb-6">
        {tabs.map((tab) => (
          <Link key={tab.id} href={`/profile?tab=${tab.id}`}
            ...
            {tab.label}
            {tab.count !== null && (
              <span className={`text-xs px-1.5 py-0.5 rounded-full ${activeTab === tab.id ? 'bg-brand-500/20 text-brand-400' : 'bg-surface-raised text-zinc-600'}`}>
                {tab.count}
              </span>
            )}
          </Link>
      ...
      {activeTab === 'wishlist' && (
        wishedProducts.length > 0 ? (
            ...
            {wishedProducts.map((item) => (
              <Link key={item.id} href={`/product/${item.id}`}
                ...
                <div className="flex items-center gap-3 shrink-0">
                  ...
                  <button
                    onClick={(e) => { e.preventDefault(); toggle(item.id, user?.id); }}
          ...
          <div className="text-center py-16 border border-dashed border-surface-border rounded-2xl">
            ...
            <Link href="/feed" className="inline-block px-4 py-2 bg-brand-500/10 border border-brand-500/20 text-brand-400 rounded-lg text-sm font-medium hover:bg-brand-500/20 transition-colors">
              피드 보러가기
            </Link>
          </div>
        )
        ...
        <div className="space-y-4">
          ...
          <div className="bg-surface border border-surface-border rounded-xl overflow-hidden">
            <div className="px-4 py-3 border-b border-surface-border">
              <h3 className="text-sm font-semibold text-white">계정 설정</h3>
            </div>
            <div className="divide-y divide-surface-border">
              ...
              <div className="flex items-center justify-between px-4 py-3.5">
                <div>
                  <p className="text-sm text-white">닉네임</p>
              ...
              <div className="flex items-center justify-between px-4 py-3.5">
                <div>
                  <p className="text-sm text-white">프로필 사진</p>
                  <p className="text-xs text-zinc-500 mt-0.5">
                    {profile?.avatar_url ? '업로드된 사진 사용 중' : '기본 이니셜 사용 중'}
                  </p>
                </div>
          ...
          <div className="bg-surface border border-surface-border rounded-xl overflow-hidden">
            ...
            <div className="divide-y divide-surface-border">
              <div className="flex items-center justify-between px-4 py-3.5">
                <p className="text-sm text-zinc-300">버전</p>
                <p className="text-sm text-zinc-500">v0.2.0</p>
              </div>
              <div className="flex items-center justify-between px-4 py-3.5">
                <p className="text-sm text-zinc-300">문의하기</p>
                <p className="text-sm text-zinc-500">kitpick@email.com</p>
              </div>
            </div>
          </div>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-surface-overlay border border-surface-border rounded-2xl p-6 w-full max-w-sm shadow-2xl">
            ...
            <p className="text-sm text-zinc-400 mb-6">찜 목록, 프로필 사진 등 모든 데이터가 영구 삭제됩니다. 이 작업은 되돌릴 수 없습니다.</p>
            <div className="flex gap-3">
              <button onClick={() => setShowDeleteModal(false)} className="flex-1 py-2.5 bg-surface-raised border border-surface-border rounded-xl text-sm font-medium text-white hover:bg-surface-overlay transition-colors">
                취소
              </button>
