...
type ArticleCategory = 'reprint' | 'release' | 'community' | 'deal' | 'review' | 'event';
...
const filterTabs: Array<{ id: FilterType; label: string; icon: React.ElementType }> = [
  { id: 'all'       as const, label: '전체',      icon: Newspaper     },
  { id: 'reprint'   as const, label: '재판 알림', icon: Bell          },
  { id: 'release'   as const, label: '신작 출시', icon: Package       },
  { id: 'community' as const, label: '커뮤니티',  icon: MessageSquare },
  { id: 'deal'      as const, label: '특가',      icon: Tag           },
  { id: 'review'    as const, label: '리뷰',      icon: MessageSquare },
];
      ...
      <div className="p-5">
        ...
        <div className="flex items-center justify-between mt-4 gap-3">
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            {article.source_url && (
              <a href={article.source_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-xs text-zinc-500 hover:text-brand-400 transition-colors">
...
export default function JournalPage() {
  ...
  const loadArticles = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        const { articles: data } = await res.json() as { articles: NewsArticle[] };
  ...
  async function handleCrawl() {
    ...
    try {
      ...
      else { console.error('[crawl]', data.error); }
    } catch (e) { console.error('[crawl]', e); }
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            {useReal && <span className="text-xs px-2 py-0.5 rounded-full bg-decision-buy/10 border border-decision-buy/20 text-decision-buy font-medium">실데이터</span>}
          </div>
          <p className="text-zinc-400">커뮤니티·공식 채널 최신 소식을 한곳에서 모아봐요.</p>
        </div>
        <button onClick={handleCrawl} disabled={crawling} className="flex items-center gap-2 px-4 py-2 rounded-lg border border-surface-border bg-surface hover:bg-surface-raised text-sm text-zinc-400 hover:text-zinc-200 transition-colors disabled:opacity-50">
