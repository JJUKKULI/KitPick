...
export function DecisionCard({ product, index = 0 }: DecisionCardProps) {
  ...
  const priceDisplay = product.price >= 1000
    ? `${product.price.toLocaleString('ko-KR')}원`
    : `$${product.price.toFixed(2)}`;
  const diffDisplay = product.price >= 1000
    ? `${Math.abs(priceDiff).toLocaleString('ko-KR')}원`
  ...
  async function handleWish(e: React.MouseEvent) {
    ...
    toast(
      willWish ? 'success' : 'info',
      willWish ? '관심 목록에 추가됐어요!' : '관심 목록에서 제거됐어요.',
      product.name,
    );
  }
  ...
  function handleLike(e: React.MouseEvent) {
    ...
    toast(willLike ? 'success' : 'info', willLike ? '좋아요!' : '좋아요 취소');
  }
